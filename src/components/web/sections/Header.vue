<template>
	<div class="container">
		<div class="w-100 group d-flex justify-content-end">
			<div class="image rounded"></div>
			<div v-if="currentText != ''" class="name rounded px-4 py-2 bg-secondary text-white text-uppercase"
				:class="{ animate: currentText.length == text.length }">
				<div class="main-text">{{ currentText }}</div>
				<div class="secondary-text">Focused on <span class="text-primary">Frontend</span></div>
			</div>
		</div>
	</div>
</template>
